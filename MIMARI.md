...
- **Split-view sonuç ekranı:** Bir sonuca tıklamak artık dosyayı açmaz — `selectedIndex` state'i set edilir, sağda bir içerik paneli açılır (liste sola daralır, `.locator-split-active`). Panel `text` alanını gösterir; dosyayı asıl görüntülemek için panel içindeki ayrı "Dosyayı Görüntüle" butonuna tıklamak gerekir. `getViewerUrl` yardımcı fonksiyonu PDF'leri doğrudan (tarayıcının kendi görüntüleyicisi), diğer her şeyi (`.doc`/`.docx` vb.) Microsoft Office Online Viewer üzerinden açar — Google Docs Viewer dış URL'lerde artık güvenilir çalışmadığı (indirmeye düştüğü) için bilerek kullanılmıyor. Panel kapatma (X) `selectedIndex`'i `null`'a döndürür, liste tekrar tam genişliğe döner.
